...
impl PyMvfVideo {
    ...
    pub fn new(file_name: &str, raw_data: Vec<u8>) -> PyMvfVideo {
        if raw_data.is_empty() {
            let c = <MvfVideo as NewSomeVideo<&str>>::new(file_name);
            return PyMvfVideo { core: c };
        } else {
            let c = <MvfVideo as NewSomeVideo2<Vec<u8>, &str>>::new(raw_data, file_name);
            return PyMvfVideo { core: c };
        }
    }
