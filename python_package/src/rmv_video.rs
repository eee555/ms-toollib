...
impl PyRmvVideo {
    ...
    pub fn new(file_name: &str, raw_data: Vec<u8>) -> PyRmvVideo {
        if raw_data.is_empty() {
            let c = <RmvVideo as NewSomeVideo<&str>>::new(file_name);
            return PyRmvVideo { core: c };
        } else {
            let c = <RmvVideo as NewSomeVideo2<Vec<u8>, &str>>::new(raw_data, file_name);
            return PyRmvVideo { core: c };
        }
    }
