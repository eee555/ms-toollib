...
//!      - `1`到`8`代表数字1到8
//!      - `10`代表未打开
//!      - `11`代表算法认为是雷（百分百正确），或玩家在游戏中标的雷（玩家认为这是雷，但玩家可能犯错）
//!      - `12`代表算法已经确定该位置不是雷，但玩家暂时还没点开，模样仍然是未打开的样子
//!      - `14`表示踩到了雷游戏失败以后显示的标错的雷对应叉雷
//!      - `15`表示踩到了雷游戏失败了对应红雷
//!      - `16`表示背景不透明的白雷，失败后显示出来的其他的雷
//!      - `18`表示局面中，由于双击的高亮，导致看起来像0的格子
//!    - 第一个索引是行，第二个索引是列。例如：高级中，`game_board[0][0]`代表最左上角位置，`game_board[15][29]`代表最右下角位置。
//!    - **注意**：游戏局面中`11`的作用类似于游戏时的标雷，但是区别在于，玩家标出的雷可能是错误的，而算法的判断一定是正确的。这两种情况都用同一个数字表示。通俗地讲，因为算法需要保证百分百的正确性，所以玩家标出来的雷，算法一个也不相信，这意味着这两种含义不可能同时出现。
//! 1. “真实局面”或“局面”
//!    - 变量名为`board: Vec<Vec>`
//!    - 值的含义
//!      - `0`代表空
...
//!    在线性代数方程`Ax=b`中：
//!    - 矩阵A的变量名为`matrix_a`，代表系数矩阵
//!    - 矩阵x的变量名为`matrix_x`，代表未知量矩阵
//!    - 矩阵b的变量名为`matrix_b`，代表常数矩阵
//!    - 变量后加上`s`代表分段；加上`ses`代表分块且分段
//! 1. “分块”
//!    计算概率时，假如把整个局面全列成一个方程式，那么，方程中的变量数等于空的边缘的格子数。一般数目较大不易求解，需要使用分块技巧来加速。
//!    直观地看，游戏局面中，贯通上下两边的空可以把游戏局面分成两块；复杂的空可以把游戏局面分割成更多块。因此计算概率时，可以独立讨论被空分割成的若干块的情况（即系数矩阵可以分块），最后汇总计算出最终结果。
//! 1. “分段”
//!    仅仅分块还是不够，一些块的边缘仍然很长。但是在块的边缘，未知格往往是两两相邻的（而不是多个未知格互相相邻，即无向图的割点很多），而且其中部分未知格往往是可以简单求出的。只要求出这些未知格并代入原方程，就可以把大方程分割成小方程（即系数矩阵为分块对角矩阵），从而加速求解。
//! ## 函数签名说明
//! Rust是一门强类型的语言，其函数签名反映了诸多信息。以下为不熟悉本语言的开发人员提供简要的说明。
//! - 变量名+冒号+格式：表明参数的格式。例如`i32`代表有符号4字节的整数、`u8`代表无符号1字节的整数；`Vec<>`代表内存分配在堆上的可变长度的向量。
//! - `mut`：代表这个参数是可变的。例如`pub fn mark_board(board: &mut Vec<Vec<i32>>)`中，会对传入的局面直接修改。如果不带`mut`，则不会修改。
//! ## API命名原则
//! 约定如下原则：
...
pub use algorithms::{
    cal_probability_cells_is_op, cal_probability, cal_probability_onboard,
